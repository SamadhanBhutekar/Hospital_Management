...
function AttendantModification_info() {
  ...
  useEffect(() => {
    ...
    const relationdata = async () => {
      try {
        ...
        const data = await response.json();
        const attdenceid =data.map((item)=>item.pid)[0];
        setattdenceid(attdenceid)
  ...
  return (
    ...
    {attid === getid &&
    (
      <div className="packagebtn" style={{disply:"none"}}>
          <div className="row justify-content-center">
              {TransactionData && (
                <Link to={`/package_charges/${TransactionData._id}`}>
